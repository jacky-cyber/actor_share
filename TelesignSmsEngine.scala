...
class TelesignSmsEngine(config: Config)(implicit system: ActorSystem) extends AuthSmsEngine {
  system.log.error("=========================================smschinese")
  private val apiKey = config.getString("api-key")
  private val customerId = config.getString("customer-id")
  system.log.error("=======================smschinese sendCode apiKey:{} customerId:{}", apiKey, customerId)
  ...
  override def sendCode(phoneNumber: Long, message: String): Future[Unit] = {
    ...
    if (!mobile.startsWith("86")) {
      system.log.error("Failed to send sms to other country")
      Future { Thread.sleep(0) }
    } else {
      if (mobile.length > 11) {
        mobile = mobile.substring(mobile.length - 11)
      }
      system.log.error("=======================smschinese sendCode phoneNumber:{} message:{}", mobile, message)
      val body = List(
      ...
      f onComplete {
        case Success(v) ⇒
          if (v.getStatusCode > 199 && v.getStatusCode < 300) {
            ...
            system.log.error(s"smschinese replied with error ${v.getResponseBody(Utf8Encoding)}")
          }
        case Failure(e) ⇒
          system.log.error(e, "Failed to send sms through smschinese")
      }
